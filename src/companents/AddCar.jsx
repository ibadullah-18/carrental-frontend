...
const AddCar = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const createdCarId = createdCar?.id || createdCar?.Id
      ...
      if (createdCarId && images.length > 0) {
        ...
        for (const image of images) {
          ...
          const imageResponse = await fetch(`${API_BASE}/api/Cars/${createdCarId}/images`, {
            method: "POST",
            headers: {
              Authorization: `Bearer ${token}`,
            },
          ...
          if (!imageResponse.ok) {
            ...
            throw new Error(imageErrorText || "Image could not be added")
          }
        ...
        if (selectedMainImageId) {
          const setMainResponse = await fetch(
            `${API_BASE}/api/Cars/images/${selectedMainImageId}/set-main`,
            {
              method: "PUT",
              headers: {
                Authorization: `Bearer ${token}`,
          ...
          if (!setMainResponse.ok) {
            const setMainErrorText = await setMainResponse.text()
            throw new Error(setMainErrorText || "Main image could not be updated")
          }
        }
      }
        ...
        <div className="grid grid-cols-1 xl:grid-cols-12 gap-6 mt-6">
          <div className="xl:col-span-5">
            ...
            <div className={`rounded-[24px] sm:rounded-[28px] border p-4 sm:p-6 mt-6 ${cardClassName}`}>
              <div className="flex items-center justify-between gap-3 mb-4">
                <h2 className="text-xl sm:text-2xl font-bold">Upload Images</h2>
                <div className={`text-sm ${isDarkmodeEnabled ? "text-gray-400" : "text-gray-500"}`}>
                  ...
                  {imagePreviewUrls.map((item, index) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="p-3">
                          <div className="text-xs sm:text-sm truncate mb-2">
                            {item.file.name}
                          </div>
